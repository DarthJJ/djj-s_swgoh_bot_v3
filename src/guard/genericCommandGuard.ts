...
export const Admin: GuardFunction<CommandInteraction> = async (
  interaction,
  client,
  next,
  guardData
...
export const BotOwner: GuardFunction<CommandInteraction> = async (
  interaction,
  client,
  next,
  guardData
...
export const PlayerRegistered: GuardFunction<CommandInteraction> = async (
  interaction,
  client,
  next,
  guardData
  ...
  interaction.reply(
    "```You are not registered with this bot, please use the register command```"
  );
};

export const CommandEnabled: GuardFunction<CommandInteraction> = async (
  interaction,
  client,
  next,
  guardData
  ...
  if (
    command &&
    (command.enabled ||
      (command.ownerOverride &&
        interaction.member!.user!.id === new Config().BOT_ADMIN_ID))
  ) {
    await next();
    return;
  }
...
export const CountryCode: GuardFunction<CommandInteraction> = async (
  interaction,
  client,
  next,
  guardData
